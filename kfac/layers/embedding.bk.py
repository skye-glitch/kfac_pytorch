...
class EmbeddingLayer(KFACLayer):
    ...
    def _get_A_factor(self, a_inputs):
        ...
        a = a ** 2 # compute the covariance
        #print(a[0][0].dtype)
        #res = torch.mean(a, dim=0)
        #print(res[0].dtype)
        return torch.sum(a, dim=0)/len(a_inputs)
    ...
    def update_A_factor(self, alpha=0.95):
        ...
        #         logging.critical("find inf in a_inputs 1 emb")
        #     break
        ...
        #     _t = i.cpu()
        #     if not np.isfinite(_t).all():
        #         logging.critical("find inf in a_inputs 2 emb")
        #     break
        A_new = self._get_A_factor(self.a_inputs)
        # for i in A_new:
        #     _t = i.cpu()
        #     if not np.isfinite(_t).all():
        #         logging.critical("find inf in A_new emb")
        #     break
        ...
        # for i in self.A_factor:
        #     _t = i.cpu()
        #     if not np.isfinite(_t).all():
        #         logging.critical("find inf in self.A_factor emb")
        #     break
    ...
    def compute_A_inv(self, damping=0.001, ignore_rank=False):
        ...
        # if not torch.isfinite(self.A_inv).all():
        #     logging.critical("find inf in self.A_inv emb")
    ...
    def _get_G_factor(self, g_inputs):
        ...
        assert len(g.shape) == 2  # shape should be (batch, num_embeddings) where batch dim
                                # has size batch_size * seq_len
        #print("G shape", len(g_inputs), g.shape, self.module.num_embeddings)
    ...
    def _get_precondition_gradient_hybrid(self):
        ...
        """
        old_grad = self.get_gradient()
        old_grad_dtype = old_grad.dtype
        grad = old_grad.to(self.inv_dtype)
    ...
    def broadcast_inverses(self):
        ...
        ops = [comm.backend.broadcast(self.A_inv, 
    ...
    def state_dict(self, include_inverses=False):
        ...
        if include_inverses:
            ...
            'G': self.state['G'], 'G_shape':self.state['G_shape'], 'G_inv': self.state['G_inv']}
        else:
            return {'A_shape':self.state['A_shape'], 'A_factor': self.A_factor,
            'G': self.state['G'], 'G_shape':self.state['G_shape']}
